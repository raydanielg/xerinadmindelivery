/*---------------------------------------------
	Template name:  Xerin Express Delivery
	Version:        1.0
	Author:         6amtech
...
(function ($) {
  ...
  08: Dark, Light & RTL Switcher
  ==============================================*/
    function setActiveSettingBox(className, groupClassNames) {
        $(groupClassNames).removeClass("active");
    ...
    function rtlSwitcher(className, dirName) {
        $(className).on("click", function () {
            setActiveSettingBox(this, ".setting-box.ltr-mode, .setting-box.rtl-mode");
    ...
    function initializeSettingsSwitcher() {
        let selectedTheme = localStorage.getItem("theme") || $("body").attr("theme") || "light";
        let selectedDir = localStorage.getItem("dir") || $("html").attr("dir") || "ltr";

        if (selectedTheme) {
            $("body").attr("theme", selectedTheme);
            setActiveSettingBox(
                ".setting-box." + selectedTheme + "-mode",
                ".setting-box.light-mode, .setting-box.dark-mode"
            );
        }

        if (selectedDir) {
            $("html").attr("dir", selectedDir);
            setActiveSettingBox(
                ".setting-box." + selectedDir + "-mode",
                ".setting-box.ltr-mode, .setting-box.rtl-mode"
            );
        }
    }

    initializeSettingsSwitcher();
    $(window).on("load", initializeSettingsSwitcher);
        ...
        function setActive(el) {
            ...
            if (el.is("button[type='reset']")) {
                el.css({
                    "color": "",
                    "outline": "none",
                    "box-shadow": "0 0 0 .18rem rgba(190, 192, 194, 0.35)",
                    "border-color": "#bec0c2"
                });
            }
