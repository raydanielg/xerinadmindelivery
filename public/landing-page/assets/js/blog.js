...
$(document).ready(function () {
    ...
    const $searchInput = $searchForm.find('.blog-search-input');
    const $suggestions = $searchForm.find('.blog-search-suggestions');
    const suggestionUrl = $searchForm.data('suggestion-url');
    ...
    function renderSuggestions(suggestions) {
        ...
        const suggestionItems = suggestions.map(function (suggestion) {
            return $('<button/>', {
                type: 'button',
                class: 'blog-search-suggestion-item',
                text: suggestion,
        ...
        $suggestions.empty().append(suggestionItems).removeClass('d-none');
    }

    if ($searchInput.length && suggestionUrl) {
        $searchInput.on('input', function () {
            ...
            debounceTimer = setTimeout(function () {
                ...
                suggestionRequest = $.getJSON(suggestionUrl, {search: searchValue})
        ...
        $suggestions.on('click', '.blog-search-suggestion-item', function () {
        ...
        $(document).on('click', function (event) {
            if (!$(event.target).closest('.blog-search-form').length) {
                hideSuggestions();
            }
        });
    }
...
$(document).ready(function () {
    ...
    $(window).on('scroll', function () {
        const $blogDescription = $('.blog-details-section .scrollspy-blog-details');
        const scrollPosition = $(window).scrollTop();
        const containerTop = $blogDescription.offset().top;
        const containerBottom = containerTop + $blogDescription.outerHeight();
