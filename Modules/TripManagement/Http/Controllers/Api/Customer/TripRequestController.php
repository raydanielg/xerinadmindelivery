...
class TripRequestController extends Controller
{
    ...
    {
        if ($request->type == "ride_request" &&  $request->trip_request_id == null){
            if ($this->tripRequestService->hasIncompleteRegularRide(data: ['type' => RIDE_REQUEST, 'ride_request_type' => 'regular', 'customer_id' => auth('api')->id()])) {
        ...
        try {
            ...
            if ($request->bid) {
                ...
                $discount = $this->getEstimatedDiscount(
                    user: $save_trip->customer,
                    zoneId: $save_trip->zone_id,
                    tripType: $save_trip->type,
                    vehicleCategoryId: $save_trip->vehicle_category_id,
    ...
    public function getEstimatedFare(GetEstimatedFaresOrNotRequest $request): JsonResponse
    {
        ...
        $recentAddressData = [
            ...
            'destination_lng' => $destinationCoordinates[0],
            'destination_lat' => $destinationCoordinates[1],
        ...
        app()->terminating(function () use ($recentAddressService, $recentAddressData) {
            try {
                $recentAddressService->create(data: [
                    'user_id' => $recentAddressData['user_id'],
                    'zone_id' => $recentAddressData['zone_id'],
                    'pickup_coordinates' => DB::raw("ST_GeomFromText('POINT({$recentAddressData['pickup_lng']} {$recentAddressData['pickup_lat']})', 4326)"),
                    'destination_coordinates' => DB::raw("ST_GeomFromText('POINT({$recentAddressData['destination_lng']} {$recentAddressData['destination_lat']})', 4326)"),
                    'pickup_address' => $recentAddressData['pickup_address'],
                    'destination_address' => $recentAddressData['destination_address'],
                ]);
            } catch (\Throwable $e) {
                Log::warning('Recent address create failed', ['error' => $e->getMessage()]);
            }
        });
    ...
    public function rideStatusUpdate($trip_request_id, Request $request): JsonResponse
    {
        ...
        try
        {
            ...
            if ($attributes['trip_status'] ?? null) {
                ...
                if ($attributes['trip_status'] == COMPLETED || $attributes['trip_status'] == CANCELLED) {
                    $trip?->tripNavigation()->delete();
                }
            }
