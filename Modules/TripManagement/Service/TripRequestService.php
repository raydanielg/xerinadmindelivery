...
class TripRequestService extends BaseService implements TripRequestServiceInterface
{
    ...
    public function getPendingRide(array $data = []): mixed
    {
        ...
        $femaleOnlyRideServiceEnabled = (bool)(businessConfig('female_only_ride_service', TRIP_SETTINGS)?->value ?? false);
        if ($femaleOnlyRideServiceEnabled && auth('api')->user()?->gender !== GENDER_FEMALE) {
            $criteria = array_merge($criteria, ['is_female_driver_requested' => 0]);
        }
    ...
    public function updateRideStatus(array $data = []): mixed
    {
        ...
        if ($data['require_delivery_proof'] ?? false) {
            $trip->proofImage()->updateOrCreate([], ['delivery_proof_images' => $data['upload_delivery_images'] ?? []]);
        }
...
//            }
    ...
    public function createRideRequest(array $attributes = []): mixed
    {
        ...
        $femaleOnlyServiceEnabled = (bool)(businessConfig(key: 'female_only_ride_service', settingsType: TRIP_SETTINGS)?->value ?? 0);
        $isFemaleDriverRequested = $femaleOnlyServiceEnabled && filter_var($attributes['is_female_driver_requested'] ?? false, FILTER_VALIDATE_BOOLEAN);

        $parcelDeliveryProofEnabled = (bool)(businessConfig(key: 'enable_parcel_delivery_proof', settingsType: PARCEL_SETTINGS)?->value ?? 0);
        $isParcelDeliveryProofEnabled = $parcelDeliveryProofEnabled && ($attributes['type'] ?? null) == PARCEL ? 1 : 0;
        ...
        try {
            $trip = $this->tripRequestRepository->create([
                ...
                'is_parcel_delivery_proof_enabled' => $isParcelDeliveryProofEnabled,
                'ride_request_type' => $attributes['ride_request_type'] ?? null,
                'is_female_driver_requested' => $isFemaleDriverRequested,
