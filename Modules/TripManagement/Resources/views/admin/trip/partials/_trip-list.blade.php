        ...
        @forelse($trips as $key => $trip)
            <tr>
                ...
                <td class="text-center trip-id"><a
                        href="{{route('admin.trip.show', ['type' => $type, 'id' => $trip->id, 'page' => 'summary'])}}">{{$trip->ref_id}}</a>
                </td>
                        ...
                        @if($trip?->safetyAlerts)
                            @foreach($trip->safetyAlerts as $alert)
                                @if($alert?->sentBy?->id == $trip->customer?->id)
                                    <img src="{{ dynamicAsset('public/assets/admin-module/img/safety-alert-shield-icon-red.png')}}"
                ...
                <td class="text-center trip-status"><span
                        class="badge badge-{{ $trip->payment_status == PAID? 'primary' : 'warning' }}">{{translate($trip->payment_status)}}</span>
                </td>
                <td class="text-center trip-status"><span
                        class="badge badge-{{ ($trip->current_status == COMPLETED || $trip->current_status == RETURNED)? 'primary' : 'warning' }}">{{translate($trip->current_status)}}</span>
                </td>
