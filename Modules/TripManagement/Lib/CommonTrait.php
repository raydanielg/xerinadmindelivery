...
trait CommonTrait
{
    ...
    public function estimatedFare($tripRequest, $routes, $zone_id, $zone, $tripFare = null, $area_id = null, $beforeCreate = false): mixed
    {
        ...
        if ($tripRequest['type'] == 'parcel') {
            ...
            $points = (int)getSession('currency_decimal_point') ?? 0;
            $vat_percent = (double)get_cache('vat_percent') ?? 1;
            $surges = $surgePriceService->fetchSurgesForZone($zone->id, $tripRequest['scheduled_at'] ?? null);

            $estimated_fare = $tripFare->map(function ($trip) use ($routes, $tripRequest, $area_id, $beforeCreate, $zone, $scheduleTripPercentage, $surgePriceService, $user, $extraFare, $points, $vat_percent, $surges) {
                $surgePrice = $surgePriceService->evaluateSurgeForCategory(surges: $surges, tripType: $tripRequest['type'], vehicleCategoryId: $trip->vehicle_category_id, scheduledAt: $tripRequest['scheduled_at'] ?? null);
