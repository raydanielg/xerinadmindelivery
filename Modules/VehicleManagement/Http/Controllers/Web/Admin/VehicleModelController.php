...
class VehicleModelController extends BaseController
{
    ...
    public function export(Request $request): View|Factory|Response|StreamedResponse|string|Application
    {
        ...
        $config = styledExportConfig(
            ...
            summary: ['Total Models' => $data->count()],
            filters: [
            ...
            fileName: 'vehicle-models-' . time() . '.xlsx',
            headings: ['name', 'description', 'brand_id', 'seat_capacity', 'maximum_weight', 'hatch_bag_capacity', 'engine', 'total_vehicles', 'is_active', 'created_at'],
        );
        return exportData($data, $request['file'], 'vehiclemanagement::admin.model.print', $config);
    }
