                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="mb-4">
                                                <label for="category_name"
                                                       class="mb-2">{{ translate('category_name') }}<span class="text-danger">*</span></label>
                                            ...
                                            <div class="mb-4">
                                                <label for="category_type"
                                                       class="mb-2">{{ translate('category_type') }}<span class="text-danger">*</span></label>
                                                <select id="category_type" name="type" class="form-control js-select cmn_focus"
                                                        required tabindex="2">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <div class="character-count">
                                                    <textarea id="short_desc" rows="5" name="short_desc"
                                                              ...
                                                              required tabindex="3">{{ old('short_desc') }}</textarea>
                                        ...
                                        <div class="col-md-6">
                                            <div class="card-body d-flex flex-column gap-3">
                                                <h5 class="text-center text-capitalize">{{ translate('category_image') }}<span class="text-danger">*</span></h5>
                                                <div class="d-flex justify-content-center">
                                                    <div class="upload-file cmn_focus rounded-10 auto profile-image-upload-file">
                                                        <input type="file" name="category_image" class="upload-file__input"
                                                               accept=".png" required tabindex="4" data-max-upload-size="{{ $maxSize }}">
                                                        <div
                                                            class="upload-file__img cmn_focus rounded-10 border-gray d-flex justify-content-center align-items-center w-150 h-150 aspect-1 p-0">
                                    ...
                                    <div class="d-flex justify-content-end gap-3">
                                        <button type="submit"
                                                class="btn btn-primary cmn_focus" tabindex="5">{{ translate('submit') }}</button>
                                    </div>
                                </div>
                            </div>
                        </form>
                                        ...
                                        <form action="javascript:;"
                                            ...
                                            <div class="input-group search-form__input_group">
                                                ...
                                                <input type="search" class="theme-input-style search-form__input"
                                                       ...
                                                       placeholder="{{translate('search_here_by_Category_Name')}}" tabindex="6">
                                            </div>
                                            <button type="submit"
                                                    class="btn btn-primary search-submit" data-url="{{ url()->full() }}" tabindex="7">{{ translate('search') }}</button>
                                        </form>

                                        <div class="d-flex flex-wrap gap-3">
                                            @can('super-admin')
                                                <a href="{{ route('admin.vehicle.attribute-setup.category.index', ['status' => request('status')]) }}"
                                                   class="btn btn-outline-primary px-3" data-bs-toggle="tooltip" data-bs-title="{{ translate('refresh') }}" tabindex="8">
                                                    <i class="bi bi-arrow-repeat"></i>
                                                </a>

                                                <a href="{{ route('admin.vehicle.attribute-setup.category.trashed') }}"
                                                   class="btn btn-outline-primary px-3" data-bs-toggle="tooltip" data-bs-title="{{ translate('manage_Trashed_Data') }}" tabindex="9">
                                            ...
                                            @can('vehicle_log')
                                                <a href="{{route('admin.vehicle.attribute-setup.category.log')}}"
                                                   class="btn btn-outline-primary px-3" data-bs-toggle="tooltip" data-bs-title="{{ translate('view_Log') }}" tabindex="10">
                                            ...
                                            @can('vehicle_export')
                                                <div class="dropdown">
                                                    <button type="button" class="btn btn-outline-primary"
                                                            data-bs-toggle="dropdown" tabindex="11">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                                                        <li><a class="dropdown-item"
                                                               href="{{route('admin.vehicle.attribute-setup.category.export', ['status' => request()->get('status') ?? 'all', 'search' => request()->get('search'), 'file' => 'excel'])}}">{{translate('excel')}}</a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            @endcan
                                                ...
                                                <tr id="hide-row-{{$category->id}}" class="record-row">
                                                    ...
                                                    <td >
                                                        <div class="avatar bg-transparent rounded">
                                                            <img src="{{ onErrorImage(
                                                                    $category?->image,
                                                                    dynamicStorage('storage/app/public/vehicle/category') . '/' . $category?->image,
                                                    ...
                                                    <td class="action">
                                                        <div
                                                            ...
                                                            @can('vehicle_delete')
                                                                <button
                                                                    data-url="{{ route('admin.vehicle.attribute-setup.category.delete', ['id' => $category->id]) }}"
                                                                    data-icon="{{ dynamicAsset('public/assets/admin-module/img/trash.png') }}"
                                                                    data-title="{{ translate('Are you sure to delete this Vehicle Category') }}?"
