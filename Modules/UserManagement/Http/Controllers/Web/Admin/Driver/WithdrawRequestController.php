...
use Modules\UserManagement\Service\Interfaces\WithdrawMethodServiceInterface;
...
class WithdrawRequestController extends BaseController
{
    ...
    public function action(string|int $id, WithdrawRequestActionRequest $request)
    {
        ...
        if ($withdrawRequest) {
            if ($withdrawRequest->method == null) {
        ...
        if (array_key_exists('status', $data) && !is_null($data['status']) && array_key_exists('ids', $data) && (count($data['ids']) > 0)) {
            if ($data['status'] == 'invoice') {
                ...
                foreach ($data['ids'] as $id) {
                    $withdrawRequest = $this->withdrawRequestService->findOne(id: $id, relations: ['user' => []]);
                    if ($withdrawRequest) {
                        if (count($data['ids']) == 1 && ($withdrawRequest->method == null || $withdrawRequest?->user?->id == null)) {
                            Toastr::error(translate("Withdraw method or driver not found"));
                            return redirect()->back();
                        } else {
                            if (!($withdrawRequest->method == null || $withdrawRequest?->user?->id == null)) {
    ...
    public function singleInvoice(int|string $id, Request $request)
    {
        ...
        if ($type != 'pdf') {
            $rows = $data->map(fn($w) => [
                'Id'         => $w->id,
                'Driver'     => trim(($w->user->first_name ?? '') . ' ' . ($w->user->last_name ?? '')) ?: ($w->user->phone ?? '—'),
                'Method'     => $w->withdrawMethod?->method_name ?? '—',
                'Amount'     => (float)$w->amount,
                'Status'     => ucfirst((string)$w->status),
                'Requested'  => optional($w->created_at)->format('Y-m-d H:i'),
                'Updated'    => optional($w->updated_at)->format('Y-m-d H:i'),
            ]);
            $config = styledExportConfig(
                $rows,
                title: 'Withdraw Request Invoice',
                summary: ['Total Amount' => $rows->sum('Amount')],
                filters: ['Withdraw ID' => $id],
                columnFormats: [
                    'Amount'    => ColumnFormat::CURRENCY,
                    'Requested' => ColumnFormat::DATETIME,
                    'Updated'   => ColumnFormat::DATETIME,
                    'Status'    => ColumnFormat::STATUS,
                ],
                fileName: 'withdraw-request-' . $id . '-' . time() . '.xlsx',
                headings: ['Driver', 'Method', 'Amount', 'Status', 'Requested', 'Updated'],
            );
            return exportData($rows, $type, 'usermanagement::admin.driver.withdraw-request.invoice', $config);
        }
