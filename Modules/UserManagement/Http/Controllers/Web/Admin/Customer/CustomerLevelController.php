...
class CustomerLevelController extends BaseController
{
    ...
    public function export(Request $request): View|Factory|Response|StreamedResponse|string|Application
    {
        ...
        $config = styledExportConfig(
            ...
            columnFormats: [
                'Targeted Ride Trip'        => ColumnFormat::INTEGER,
                'Targeted Ride Point'       => ColumnFormat::INTEGER,
                'Minimum Spend Amount Cash' => ColumnFormat::CURRENCY,
                'Minimum Spend Amount Point'=> ColumnFormat::INTEGER,
                'Ride Cancellation Rate'    => ColumnFormat::DECIMAL,
                'Ride Cancellation Point'   => ColumnFormat::INTEGER,
                'Minimum Review'            => ColumnFormat::DECIMAL,
                'Minimum Review Point'      => ColumnFormat::INTEGER,
            ],
            fileName: 'customer-levels-' . time() . '.xlsx',
            headings: ['Name', 'Targeted Ride Trip', 'Targeted Ride Point', 'Minimum Spend Amount Cash', 'Minimum Spend Amount Point', 'Ride Cancellation Rate', 'Ride Cancellation Point', 'Minimum Review', 'Minimum Review Point', 'Completed Rides', 'Max Cancellation Rate', 'Total Customer', 'Status'],
        );
        return exportData($data, $request['file'], 'usermanagement::admin.customer.level.print', $config);
    }
