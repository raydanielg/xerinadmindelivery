...
class DriverService extends BaseService implements Interfaces\DriverServiceInterface
{
    ...
    public function create(array $data): ?Model
    {
        ...
        if (array_key_exists('identity_images', $data)) {
            foreach ($data['identity_images'] as $image) {
                $identityImages[] = fileUploader('driver/identity/', APPLICATION_IMAGE_FORMAT, $image);
            }
        }
        ...
        if (array_key_exists('other_documents', $data)) {
            ...
            foreach ($data['other_documents'] as $document) {
                $otherDocuments[] = fileUploader('driver/document/', AdditionalDataForm::uploadFormat($document), $document);
            }
        }
        $driverData = array_merge($data, [
            ...
            'profile_image' => array_key_exists('profile_image', $data) ? fileUploader('driver/profile/', APPLICATION_IMAGE_FORMAT, $data['profile_image']) : null,
    ...
    public function update(int|string $id, array $data = []): ?Model
    {
        ...
        if (array_key_exists('profile_image', $data)) {
            $profile_image = fileUploader('driver/profile/', APPLICATION_IMAGE_FORMAT, $data['profile_image'], $driver?->profile_image);
        }
        ...
        if (array_key_exists('type', $data)) {
            ...
            if ($data['other_documents'] ?? null) {
                foreach ($data['other_documents'] as $doc) {
                    $documents[] = fileUploader('driver/document/', AdditionalDataForm::uploadFormat($doc), $doc);
                }
            }
    ...
    public function getChattingDriverList(array $data): Collection
    {
        ...
        return $this->userRepository->getChattingDriverList(criteria: $criteria, searchCriteria: $searchData, whereInCriteria: $whereInCriteria, relations: $relations, whereHasRelations: $whereHasRelations);
    }
