            ...
            @can('promotion_view')
                <div class="d-flex flex-wrap justify-content-between gap-2 align-items-center mb-3 mb-sm-2">
                    ...
                    <div>
                        ...
                        <select name="date_range" id="dateRange" class="js-select date-range-change"
                                data-url="{{url()->full()}}">
                            <option
                                    value="{{ALL_TIME}}"
                ...
                <div class="row g-4">
                    <div class="col-lg-6">
                        <div class="card border-0 h-100">
                            <div class="card-body">
                                <h6 class="mb-3">{{$dateRangeValue != null ? translate($dateRangeValue) : translate(ALL_TIME) }}
                                    <i class="bi bi-arrow-up-circle text-primary ms-2"></i></h6>
                                <div class="row g-4">
                                    <div class="col-sm-6">
                                        <div class="card m-0">
                                                        ...
                                                        <div class="level-status fs-5 p-2 bg-primary w-36 aspect-1">
                                                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                                                                ...
                                                                <path d="M6.99984 9.33329C6.92323 9.33331 6.84736 9.31824 6.77658 9.28893C6.7058 9.25962 6.64149 9.21665 6.58731 9.16248C6.53314 9.10831 6.49018 9.044 6.46087 8.97321C6.43156 8.90243 6.41648 8.82657 6.4165 8.74996V5.24996C6.4165 5.09525 6.47796 4.94688 6.58736 4.83748C6.69675 4.72808 6.84513 4.66663 6.99984 4.66663C7.15455 4.66663 7.30292 4.72808 7.41232 4.83748C7.52171 4.94688 7.58317 5.09525 7.58317 5.24996V8.74996C7.58319 8.82657 7.56812 8.90243 7.53881 8.97321C7.5095 9.044 7.46653 9.10831 7.41236 9.16248C7.35819 9.21665 7.29388 9.25962 7.22309 9.28893C7.15231 9.31824 7.07645 9.33331 6.99984 9.33329Z"
                                                                      fill="white"/>
                                                                <path d="M5.35026 7.52266C5.75297 7.52266 6.07943 7.19621 6.07943 6.7935C6.07943 6.39079 5.75297 6.06433 5.35026 6.06433C4.94755 6.06433 4.62109 6.39079 4.62109 6.7935C4.62109 7.19621 4.94755 7.52266 5.35026 7.52266Z"
                                                                      fill="white"/>
                                                                <path d="M8.64958 7.93563C9.05228 7.93563 9.37874 7.60917 9.37874 7.20646C9.37874 6.80375 9.05228 6.47729 8.64958 6.47729C8.24687 6.47729 7.92041 6.80375 7.92041 7.20646C7.92041 7.60917 8.24687 7.93563 8.64958 7.93563Z"
                                                                      fill="white"/>
                                                            </svg>
                                                        </div>
                                                    </div>
                        ...
                        <div class="card h-100 border-0 mb-3">
                            <div class="card-header d-flex flex-wrap justify-content-between gap-10">
                                <div class="d-flex flex-column gap-1">
                                    ...
                                    <p>{{translate('Monitor coupon statistics')}}</p>
                                </div>
                                <div class="d-flex flex-wrap flex-sm-nowrap gap-2 align-items-center">
                                    <h6
                                            class="fs-12 text-dark">{{$dateRangeValue != null ? translate($dateRangeValue) : translate(ALL_TIME) }}</h6>
                                </div>
                            </div>
                            <div class="">
                        ...
                        <div class="card-body">
                            <div class="table-top d-flex flex-wrap gap-10 justify-content-between">
                                <form action="{{url()->full()}}"
                                    ...
                                    <button type="submit"
                                            class="btn btn-primary cmn_focus">{{ translate('search') }}</button>
                                </form>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                                                <li><a class="dropdown-item"
                                                       href="{{route('admin.promotion.coupon-setup.export', ['status' => request()->get('status') ?? 'all', 'search' => request()->get('search'), 'file' => 'excel'])}}">{{translate('excel')}}</a>
                                                </li>
                                            </ul>
                                        </div>
                                    @endcan
                            ...
                            <div class="table-responsive mt-3">
                                <table class="table table-borderless align-middle table-hover text-nowrap text-center">
                                    ...
                                    <tr>
                                        ...
                                        <th class="module">{{ translate('category') }}</th>
                                    ...
                                    @forelse ($coupons as $key => $coupon)
                                        <tr id="hide-row-{{$coupon->id}}" class="record-row">
                                            ...
                                            <td class="customer_level">
                                                ...
                                                @else
                                                    @foreach($coupon->customerLevels as $key => $level)
                                                        <span
                                                                class="badge bg-warning rounded-pill badge-sm text-capitalize  mb-2">{{ $level->name }}</span>
                                                        @if($key % 2 == 1)
                                                            <br>
                                                        @endif
                                                    @endforeach
                                            ...
                                            <td class="customer">
                                                ...
                                                @else
                                                    @foreach($coupon->customers as $key => $customer)
                                                        <span
                                                                class="badge bg-success rounded-pill badge-sm text-capitalize  mb-2">{{ $customer->first_name }}</span>
                                                        @if($key % 2 == 1)
                                                            <br>
                                                        @endif
                                                    @endforeach
                                                    ...
                                                    <label class="switcher mx-auto">
                                                        @if($date->gt($coupon->end_date))
                                                            ...
                                                            <span class="switcher_control"
                                                                  title="{{ translate('Coupon already expired, You do not change this status.') }}"></span>
                                                        @else
                                                            <input class="switcher_input status-change"
                                                                   data-url={{ route('admin.promotion.coupon-setup.status') }} id="{{ $coupon->id }}"
                                                ...
                                                <div class="d-flex justify-content-center gap-2 align-items-center">
                                                    ...
                                                    @can('promotion_delete')
                                                        <button title="{{ translate('delete_coupon') }}"
                                                                data-url="{{ route('admin.promotion.coupon-setup.delete', ['id' => $coupon->id]) }}"
                                                                data-icon="{{ dynamicAsset('public/assets/admin-module/img/trash.png') }}"
                                                                data-title="{{ translate('Are you sure to delete this Coupon') }}?"
                                                                data-sub-title="{{ translate('Once you delete it') . ', ' . translate('You can recover it later from the Deleted Coupon section.') }}"
